...
class StringCompareJaroWinkler
{
    ...
    public function JaroWinkler($string1, $string2, $boostThreshold, $prefixLength = 4, $prefixScale = 0.1)
    {
        $JaroDistance = $this->Jaro($string1, $string2);

        if ($JaroDistance < $boostThreshold) return $JaroDistance;

        $prefixLength = $this->getPrefixLength($string1, $string2, $prefixLength);
